...
#include <fileio.h>
extern "C" {
#include "../modules/conquestserv/conquest_service.h"
}
...
static int lua_writefile(lua_State *L){
	...
	size_t lenn;
	const char *text = luaL_checklstring(L, 2, &lenn);
...
static SifRpcClientData_t sc2_rpc;
...
static int sc2_rpcbind(lua_State *L){
	...
	while(1)
	{
		if((E = SifBindRpc(&sc2_rpc, SC2MCSERV_IRX, 0)) < 0)
        {
            printf("Failed to bind RPC server for SC2MCSERV (%d)\n", E);
			return -1;
        }

		if(sc2_rpc.server != NULL)
			break;

		nopdelay();
		if (!retry--) luaL_error(L, "Failed to connect with conquest card manager RPC service");
	}
	lua_pushinteger(L, 1);
	rpc_initialized = 1;
...
static int sc2_authcard(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	if (argc < 2) return luaL_error(L, "wrong number of arguments (need 2)");
    conquest_packet_t pkt;
    memset(&pkt, 0, sizeof(pkt));
    pkt.port = luaL_checkinteger(L, 1);
    pkt.slot = luaL_checkinteger(L, 2);
    if (SifCallRpc(&sc2_rpc, SC2_AUTHCARD, 0, RPCBUFF_PARAMS(pkt), NULL, NULL) < 0)
    {
        printf("%s: RPC ERROR\n", __FUNCTION__);
		lua_pushnil(L);
        return 1;
...
static int sc2_readpage(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	int bring_ecc = 0;
	if (argc < 3) return luaL_error(L, "wrong number of arguments (need 3)");
	...
	if (argc > 3) bring_ecc = luaL_checkinteger(L, 4);

    if (SifCallRpc(&sc2_rpc, SC2_READPAGE, 0, RPCBUFF_PARAMS(pkt), NULL, NULL) < 0)
    {
        printf("%s: RPC ERROR\n", __FUNCTION__);
		lua_pushnil(L);
        return 1;
	...
	lua_pushinteger(L, pkt.ret);
    //hexdump(pkt.page.full, (bring_ecc) ? MEMORYCARD_PAGESIZE_ECC:MEMORYCARD_PAGESIZE, 1);
    if (bring_ecc) {
        lua_pushlstring(L, (const char*)pkt.page.full, MEMORYCARD_PAGESIZE_ECC);
    } else {
        lua_pushlstring(L, (const char*)pkt.page.full, MEMORYCARD_PAGESIZE);
    }
    return 2;
}

static int sc2_writepage(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	size_t newpagesize;
    ...
    pkt.slot = luaL_checkinteger(L, 2);
    pkt.pagenum = luaL_checkinteger(L, 3);
	const char *newpagedata = luaL_checklstring(L, 4, &newpagesize);
	if (newpagesize < MEMORYCARD_PAGESIZE) {
...
static int sc2_check_card_magic(lua_State *L) {
	...
	const char SC2MC_MAGIC[] = "Memory Card for SoulCaliburII (C)1995 1998 2002 NAMCO LTD.";
	if (0==strncmp(PS2MC_MAGIC, (char*)pkt.page.full, sizeof(PS2MC_MAGIC)-1)) {
		ret = 1;
	}
	if (0==strncmp(SC2MC_MAGIC, (char*)pkt.page.full, sizeof(SC2MC_MAGIC)-1)) {
		ret = 2;
	}
	lua_pushinteger(L, ret);
    return 1;
...
static int sc2_verifypage(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	if (argc < 3) return luaL_error(L, "wrong number of arguments (need 3)");
...
static int sc2_eraseblock(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	if (argc < 3) return luaL_error(L, "wrong number of arguments (need 3)");

    eraseblock_packet_t pkt;
    memset(&pkt, 0, sizeof(pkt));

    pkt.port = luaL_checkinteger(L, 1);
    pkt.slot = luaL_checkinteger(L, 2);
    pkt.blocknum = luaL_checkinteger(L, 3);

    if (SifCallRpc(&sc2_rpc, GENERIC_ERASEBLOCK, 0, RPCBUFF_PARAMS(pkt), NULL, NULL) < 0)
    {
        printf("%s: RPC ERROR\n", __FUNCTION__);
		lua_pushinteger(L, 0);
        return 1;
...
static int sc2_get_cardspecs(lua_State *L) {
    CHECK_RPC_INIT();
	int argc = lua_gettop(L);
	if (argc < 2) return luaL_error(L, "wrong number of arguments (need 2)");
    ...
    if (SifCallRpc(&sc2_rpc, GENERIC_GET_CARDSPECS, 0, RPCBUFF_PARAMS(pkt), NULL, NULL) < 0)
    {
        printf("%s: RPC ERROR\n", __FUNCTION__);
		lua_pushnil(L);
        return 1;
	...
	lua_pushstring(L, "pagesize");
	lua_pushinteger(L, pkt.pagesize);
	lua_settable(L, -3);

	lua_pushstring(L, "blocksize");
	lua_pushinteger(L, pkt.blocksize);
	lua_settable(L, -3);

	lua_pushstring(L, "cardsize");
	lua_pushinteger(L, pkt.cardsize);
	lua_settable(L, -3);

	lua_pushstring(L, "cardflags");
	lua_pushinteger(L, pkt.cardflags);
	lua_settable(L, -3);
...
static const luaL_Reg Sc2_functions[] = {
	...
	{"verify_page",          sc2_verifypage},
	{"get_cardspecs",        sc2_get_cardspecs},
//	{"convert_card",         sc2_convertcard},
//	{"repair_card",          sc2_repaircard},
...
void luaSystem_init(lua_State *L) {
	...
	lua_newtable(L);
	luaL_setfuncs(L, Sc2_functions, 0);
	lua_setglobal(L, "Conquest");
